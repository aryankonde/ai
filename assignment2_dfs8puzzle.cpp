...
void solveWithDFS(int initialBoard[3][3]) {
    ...
    while (!stack.empty()) {
        PuzzleState* current = stack.back();
        stack.pop_back();
        statesExplored++;
        ...
        reverse(moves.begin(), moves.end()); // ensure DFS order (right, left, down, up)
